...
GROUP_INDEX = os.environ.get("GROUP_INDEX", "")
if GROUP_INDEX:
...
class DanmakuCollector:
    ...
    def __init__(self, room_id, anchor_name, output_dir):
        ...
        self.data = {"viewer_counts": [], "http_viewer_counts": [], "total_user_strs": [], "danmaku": [], "pw_start": 0}
    ...
    def save_data(self):
        """Write collected data to disk, returns (pwc_viewer_counts, danmaku, peak_vc, peak_tu)."""
        pw_start = self.data.get("pw_start", 0)
        # Peak from HTTP viewer counts only
        hvc_list = self.data.get("http_viewer_counts", [])
        peak_vc = max((v["count"] for v in hvc_list), default=0)
        peak_tu = ""
        _tus = self.data.get("total_user_strs", [])
        if _tus:
            peak_tu = _tus[-1].get("str", "")
        ...
        log(f"[PW] {self.anchor_name} saved: {len(self.data['danmaku'])} danmaku, "
            f"{len(hvc_list)} http_vc, peak={peak_vc}, "
            f"total_u={len(self.data.get('total_user_strs',[]))}")
        return self.data["viewer_counts"], self.data["danmaku"], peak_vc, peak_tu
    def _run(self):
        try:
            from playwright.sync_api import sync_playwright
        except ImportError:
        ...
        try:
            ...
            # Async probe (non-blocking, diagnostic only)
            try:
                ...
                try:
                    ...
                    if vc is not None:
                        if not self.data["viewer_counts"] or \
                           self.data["viewer_counts"][-1]["count"] != vc:
                            ...
                            _seen_texts.clear()
                    # 1b. HTTP viewer count via web/enter/ API (supports total_user_str)
                    _http_vc = None
                    if _collect_iter % 10 == 0 and self._api_url and self._api_cookies:
                        try:
                            ...
                            # 1. total_user_str - extract FIRST, independent of int() parsing
                            _tu = _dd.get('stats', {}).get('total_user_str')
                            if _tu and (not self.data["total_user_strs"] or
                               self.data["total_user_strs"][-1].get("str") != _tu):
                                self.data["total_user_strs"].append({
                                    "str": _tu, "offset": offset, "wall_ts": wall_ts
                                })
                            ...
                            try:
                                ...
                                pass
                            # 3. display_value (from room_view_stats, if available)
                            if _http_vc is None:
                                try:
                                    _dv = _dd.get('room_view_stats', {}).get('display_value')
                                    if _dv is not None:
                                        _http_vc = int(_dv)
                                except:
                                    pass
                            # 4. stats.user_count_str with _parse_viewer_count (handles "万")
                            if _http_vc is None:
                                _raw = _dd.get('stats', {}).get('user_count_str')
                                if _raw is not None:
                                    try:
                                        _http_vc = _parse_viewer_count(_raw)
                                    except:
                                        pass
                            # 5. top-level user_count_str (last resort)
                            if _http_vc is None and _dd.get('user_count_str'):
                                try:
                                    _http_vc = _parse_viewer_count(_dd['user_count_str'])
                                except:
                                    pass
                        except:
                            pass
                    ...
                    try:
                        ...
                        pass
                # Crash recovery: if no danmaku for 60s and it's been >120s since start, try page reload
                if _collect_iter % 60 == 0 and len(self.data["danmaku"]) > 0:
                    _last_dm_ts = max(d.get("wall_ts", 0) for d in self.data["danmaku"])
                    _stalled = (wall_ts - _last_dm_ts) > 60 and (wall_ts - self.data["pw_start"]) > 120
                    if _stalled:
                        log(f"[PW] {self.anchor_name} danmaku stalled for 60s+, reloading page...")
                        try:
                            page.goto(f"https://live.douyin.com/{self.room_id}", wait_until="domcontentloaded", timeout=30000)
...
def start_recording(url, quality, room_id, anchor_name="", device="unknown"):
    """Start ffmpeg segmented recording + Playwright data collector."""
    rec_start = time.time()  # Pin BEFORE ffmpeg for accurate danmaku alignment
    ...
    dev_icon = {"mobile": "📱", "pc": "🖥️"}.get(device, "")
    log(f"Start recording: {anchor_name}/{base}_%03d.mp4 [{quality}] {dev_icon}seg={seg_duration}s + audio")
    ...
    collector.start()
    return proc, outfile_pattern, audio_proc, audiofile, collector, seg_duration, rec_start
def stop_proc(proc):
...
def _build_danmaku_srt(seg_dm, seg_duration, viewer_counts=None):
    ...
    # Pre-process viewer_counts into a lookup
    vc_at_time = {}
    if viewer_counts:
    ...
    while win_start < seg_duration:
        ...
        if vc_times:
            ...
            if vc_for_win is not None and vc_for_win > 0:
                texts.append("☁ " + "{:,}".format(vc_for_win) + " 人在看")
...
def handle_room_end(rid, recordings, anchor_names, now):
    ...
    if collector:
        collector.stop()
        _, _, peak_vc, peak_tu = collector.save_data()
    ...
    try:
        if os.path.exists(meta_path):
            ...
            meta["device_type"] = rec.get("device", "unknown")
            meta["total_user_str"] = peak_tu
    ...
    start_ts_fmt = (_dt.fromtimestamp(rec.get("start", 0)) + timedelta(hours=7)).strftime("%Y%m%d_%H%M%S")
    end_ts_fmt = (_dt.fromtimestamp(now) + timedelta(hours=7)).strftime("%Y%m%d_%H%M%S")
    ...
    for seg_fname in seg_files:
        ...
        seg_end = (_dt.fromtimestamp(os.path.getmtime(seg_path)) + timedelta(hours=7)).strftime("%Y%m%d_%H%M%S")
    ...
    if os.path.exists(global_data_path):
        try:
            ...
            # Also get viewer counts for SRT
            ...
            if all_dm:
                ...
                # Build viewer_counts with _offset too
                vc_with_offset = []
                for vp in all_vc:
                    wt2 = vp.get("wall_ts", vp.get("offset", 0))
                    off2 = max(0, wt2 - rec_start)
                    vc_with_offset.append({**vp, "_offset": off2})
                vc_with_offset.sort(key=lambda x: x["_offset"])
                srt_content = _build_danmaku_srt(dm_with_offset, total_duration, vc_with_offset)
...
def run():
    global _iter_watchdog, _last_room_check
    ...
    for ri, r in enumerate(rooms):
        ...
        init_results.append((r, live, reason, url, quality, aname))
        safe_name = r['name'][:20]
        ...
        if live and url:
            device = detect_device(url)
            proc, outfile, audio_proc, audiofile, collector, seg_duration, rec_start_actual = start_recording(url, quality, r['id'], aname, device)
            recordings[r['id']] = {"proc": proc, "outfile": outfile, "audio_proc": audio_proc,
                               "audiofile": audiofile, "start": rec_start_actual,
                               "collector": collector, "seg_duration": seg_duration, "device": device}
    ...
    while True:
        try:
            ...
            if now - last_refresh > 30:
                ...
                for nr in new_rooms:
                    if nr["id"] not in [r["id"] for r in rooms]:
                        ...
                        if live and url:
                            device = detect_device(url)
                            proc, outfile, audio_proc, audiofile, collector, seg_duration, rec_start_actual = start_recording(url, quality, nr["id"], aname, device)
            ...
            if detect_rooms:
                ...
                if live and rid not in recordings:
                    if url:
                        ...
                        device = detect_device(url)
                        proc, outfile, audio_proc, audiofile, collector, seg_duration, rec_start_actual = start_recording(url, quality, rid, aname, device)
            ...
            for rid in sorted(recordings.keys()):
                ...
                _vc = ''
                if _coll:
                    # Prefer http_viewer_counts (API, potentially exact), fallback viewer_counts (PW)
                    _hvc = _coll.data.get('http_viewer_counts', [])
                    if _hvc:
                        _vc = f' [{_hvc[-1]["count"]}人]'
                    else:
                        _vcl = _coll.data.get('viewer_counts', [])
                        if _vcl:
                            _vc = f' [{_vcl[-1]["count"]}人]'
                _tus = _coll.data.get('total_user_strs', [])
                if _tus:
                    _vc += ', 累计' + _tus[-1].get('str', '')
                _dev = recordings[rid].get('device', 'unknown')
                _dicon = {'mobile': '📱', 'pc': '🖥️'}.get(_dev, '')
                log(f'[REC] {safe_rid} 已录制🔴{rh}时{rm}分{_dicon}{_vc}, 跳过检测')
            for rid in list(recordings.keys()):
            ...
            if '_meta.' in fname_only and fname_only.endswith('.json'):
                try:
                    ...
                    if 'peak_viewers' in meta_content:
                        _rb = {"pv": meta_content["peak_viewers"]}
                    if meta_content.get("total_user_str"):
                        _rb["tv"] = meta_content["total_user_str"]
                    if meta_content.get("device_type"):
                        _rb["dt"] = meta_content["device_type"]
                    if GROUP_INDEX:
                        _rb["g"] = GROUP_INDEX
                    rel_data["body"] = json.dumps(_rb)
                except:
                    pass
...
def _write_status_json(status_data):
    if not GH_REPO or not GH_TOKEN_ROOMS:
        return
    path = f'docs/live_status_{GROUP_INDEX}.json' if GROUP_INDEX else 'docs/live_status.json'
